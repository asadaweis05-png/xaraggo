/* ==========================================================================
   BILIC - Quruxda iyo Labbiska Soomaaliyeed (AI Features & Logic Engine)
   ========================================================================== */
...
function runAISkinAnalysis() {
  ...
  if (prompt.includes('finan') || prompt.includes('acne')) {
    ...
    identifiedIssues = ['Biyo la’aanta maqaarka', 'Barrier-ka oo xasaasi ah'];
    ...
    container.innerHTML = forumPosts.map(post => `
      <div class="forum-card">
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; font-size: 0.85rem;">
          ...
          <div>
            <button class="pill-btn" onclick="this.innerText = '❤️ ' + (${post.likes} + 1)"><i class="fa-regular fa-heart"></i> ${post.likes}</button>
...
async function processFinalOrder() {
  if (typeof createOrder === 'function' && state.authUser) {
    ...
    const orderNum = order?.order_number || 'BILIC-' + Math.floor(10000 + Math.random() * 90000);
    ...
    alert('🎉 Dalabkaaga Waa La Guddoomay! Nambarka raadraaca waa #BILIC-84920. Mahadsanid!');
  }
  state.cart = [];
  renderCart();
  navigateTo('home');
