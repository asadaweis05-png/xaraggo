...
async function loadProductsFromDB() {
  try {
    ...
    console.log(' Products table not found, using local data');
  }
  return false;
}
...
async function loadCartFromDB() {
  ...
  try {
    ...
    console.log(' Cart table not available');
  }
}
...
async function loadWishlistFromDB() {
  ...
  try {
    ...
    console.log(' Wishlist table not available');
  }
}
...
async function createOrder(paymentMethod) {
  ...
  try {
    ...
    console.log(' Orders table not available, processing locally');
...
async function saveSkinLogToDB(logEntry) {
  ...
  try {
    ...
    console.log(' Skin logs table not available');
  }
}
...
async function loadSkinLogsFromDB() {
  ...
  try {
    ...
    console.log(' Skin logs not available');
  }
}
...
async function handleSignUp(e) {
  ...
  if (!name || !email || !password) {
    showAuthToast(' Fadlan buuxi dhammaan meelaha.');
    return;
  }
  if (password !== confirmPassword) {
    showAuthToast(' Furaha sirta ahi isma mid ahayn.');
    return;
  }
  if (password.length < 6) {
    showAuthToast(' Furaha sirta ahi waa inuu ugu yaraan 6 xaraf ahaadaa.');
    return;
  }
...
async function handleLogin(e) {
  ...
  if (!email || !password) {
    showAuthToast(' Fadlan geli emailka iyo furaha sirta.');
    return;
  }
...
async function handleForgotPassword(e) {
  ...
  if (!email) {
    showAuthToast(' Fadlan geli emailka.');
    return;
  }
...
async function initSupabaseApp() {
  ...
  if (!supabaseClient) {
    ...
    showAuthToast(' Database connection: Offline mode');
    return;
  }
